...
fn convert_escapes(str: Option<String>) -> Option<String> {
    match str {
        Some(str) => {
            ...
            // I don't know how I feel about this code. Either it
            // is using peek() without mutable borrow issues,
            // which it is, or it is horrendous. Thinking maybe
            // it is both.
            while let Some(char) = chars.next() {
                if char == '\\' && {
                    let next = chars.peek(); // This has to be out of scope...
                    next.is_some() && {
